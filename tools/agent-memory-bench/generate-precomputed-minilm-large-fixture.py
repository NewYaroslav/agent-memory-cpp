#!/usr/bin/env python3
"""Generate the larger frozen MiniLM precomputed embedding benchmark fixture.
...
import precomputed_fixture_large_contract as content_contract


GENERATOR_REVISION = "agent-memory-cpp:minilm-l6-v2-large-fixture-v1"
GENERATOR_COMMAND = (
    "python tools/agent-memory-bench/generate-precomputed-minilm-large-fixture.py "
    "--output tests/eval/fixtures/precomputed-embedding-minilm-l6-v2-large.json"
)
DATASET_REVISION = "agent-memory-minilm-large-fixture:2026-07-22"
QRELS_REVISION = "agent-memory-minilm-large-qrels:2026-07-22"
...
def content_contract_script_path() -> Path:
    return Path(__file__).with_name("precomputed_fixture_large_contract.py")


def sha256_contract_sources() -> str:
    """Hash both the canonical encoding helper and this fixture's content."""

    payload = (
        minilm_common.sha256_file(canonical_contract_script_path())
        + "\n"
        + minilm_common.sha256_file(content_contract_script_path())
        + "\n"
    ).encode("ascii")
...
def build_fixture(
    *,
    ...
    root: dict[str, Any] = {
        "schema_version": 1,
        "name": "precomputed-embedding-minilm-l6-v2-large",
        "embedding_model": {
