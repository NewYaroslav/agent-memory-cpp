...
2. Если key уже указывает на тот же `UnitId`, операция становится idempotent
   update для этого unit. Если key указывает на другой `UnitId`, write
   конфликтует: default policy возвращает duplicate/conflict result without
   writes; explicit merge/supersede policy должна быть выбрана downstream до
   входа в storage transaction.
3. Envelope в `knowledge_units` (DBI:
   `KeyValueTable<UnitId, KnowledgeUnitEnvelope>`).
4. Mapping `KnowledgeUnitKey -> UnitId` в `content_key_to_unit_id`.
5. `knowledge_units_by_kind` update: добавить
   `(scope_id, kind) -> UnitId`; при смене kind удалить старую `(scope_id,
   old_kind) -> UnitId`.
6. Ровно один per-kind payload, соответствующий `KnowledgeUnitKind`: write в
   ...
   удаляется в той же транзакции.
7. Tag-prefixed operational components в `unit_components` (DBI:
   `TypeDiscriminatedTable<ComponentKind, UnitId, ValueVariant<UsageStats |
   Speaker | Temporal | EmbeddingMeta | CompactionMeta>>`).
8. Projections в `unit_projections` (DBI:
   `KeyValueTable<(scope_id, UnitId, ProjectionKind, revision),
   SearchProjection>`).
9. Embedding metadata + vectors (`embedding_meta`, `embedding_vectors`).
10. Все scope-aware secondary/range indexes, относящиеся к данной write:
    `inverted_token_to_unit` (по каждой projection), `field_to_postings`,
...
11. Optional downstream side effects, если профиль их включает: source refs,
...
lexical_token_stats                   KeyValueTable<CompositeKey<ScopeId, TokenId, ProjectionKind>, LexicalTokenStats>
...
| `lexical_token_stats` | по capability `LexicalIndex` | `agent_memory.lex_token_stats.v1` | TBD | Token corpus stats by `(scope, token, projection)` |
...
| Opt-in response cache delta | 0 by default, +1 if `enable_response_cache=true` persists locally | +1 | KV supported | `response_cache`; runtime-services-roadmap.md owns policy and safety defaults. |
...
of the +5 queue delta. Required key shapes:
...
std::vector<Event> MdbxTemporalIndex::fresh(ScopeId scope,
    ...
    return decode_events(observed_at_index_.range_page(from, to, std::nullopt,
                                                       n, txn).items);
}
```
