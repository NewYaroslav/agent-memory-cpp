...
2. **Compare against existing in-code surfaces.** Для каждой техники явно показать её положение относительно roadmap-label PR #29 (BinarySignature) ([`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md) §1, Planned API — not yet implemented), RFF-density pre-filter ([`memory-routing-roadmap.md`](memory-routing-roadmap.md) §4), и MinHash + LSH ([`code-intelligence-roadmap.md`](code-intelligence-roadmap.md) Pattern 1, related to roadmap-label PR #1 in that guide's roadmap numbering).
...
roadmap-label PR #29 (BinarySignature) — текущая planned binary-surface (roadmap-only, not yet implemented; см. scope note в [`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md)). Это **chunk-level** fingerprints для bucket prefilter.
