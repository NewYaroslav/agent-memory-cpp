...
2. **Compare against existing in-code surfaces.** Для каждой техники явно показать её положение относительно PR #29 binary signatures ([`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md) §1, Planned API — not yet implemented), RFF-density pre-filter ([`memory-routing-roadmap.md`](memory-routing-roadmap.md) §4), и MinHash + LSH ([`code-intelligence-roadmap.md`](code-intelligence-roadmap.md) Pattern 1, related to PR #1 in that guide's roadmap numbering).
...
- [`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md) — PR #29 binary signatures как текущая binary-surface; этот гайд покрывает техники **за её пределами**.
...
В режиме 1B (metric band search) вместо фиксированных top-L читается полоса `d_H(x, a_j) ∈ [δ_j - ρ, δ_j + ρ]`, где `δ_j = d_H(q, a_j)`. При полном dense postings это даёт **точный radius search**, а не ANN.
...
- **Бинарные коды чанков уже есть** (PR #29 binary signatures или LSH baseline).
...
- Требуется **точный top-K** без семантических anchors (SAHI 1B exact radius search возможен, но только при dense postings и подходящем anchor set).
...
|---|---|---|
| **PR #29 binary signatures** (bucket index) | [`optimization-roadmap.md`](optimization-roadmap.md) §"Binary Bucket Index Tasks" | PR #29 = bucket prefilter на коротких signatures, scope-aware. SAHI = retrieval index на длинных 128/256-bit кодах с semantic-interpretable anchors. Это **разные гранулярности**, не конкуренты. SAHI мог бы работать поверх PR #29 как второй routing-уровень (postings читают buckets второго уровня). |
...
- `sahi_postings` (DBI) — anchor-bucketed chunk IDs. Schema: `[anchor_id : uint16][bucket_distance : uint8][chunk_id : uint32]`. Bucketed layout даёт O(1) к offset-array без binary search.
...
| **PR #29 binary signatures** | [`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md) §1 | PR #29 = chunk-level signature для bucket prefilter. ASMS = document-level semantic presence sketch. ASMS оперирует documents, не chunks; иначе вычисляет, иначе индексирует. См. MSBSE §3.3. |
...
Soft binarization при обучении: `b̃ = tanh(β · l)` с постепенным увеличением `β` (continuation). Hard binarization при inference: `sign(l)` через Straight-Through Estimator. Жёсткий `min` по слотам при inference можно заменить мягким `logsumexp` при обучении с temperature `τ_s`:
...
|---|---|---|
| **PR #29 binary signatures** | [`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md) §1 | PR #29 = chunk-level signatures для bucket prefilter. MSBSE = document-level **multi-slot** semantic signatures. MSBSE — это **orthogonal axis**: multi-slot на document-level, а не single-slot на chunk-level. |
...
| **Comparison vs RFF / binary embeddings (PR #29)** | RFF = pre-filter в float RFF space; SAHI = ретривер в Hamming space; complementary. PR #29 = chunk-level signatures; SAHI = retrieval index на chunk signatures | PR #29 = chunk-level fingerprints; ASMS = document-level sketch; разные granularity | PR #29 = chunk-level single code; MSBSE = document-level multi-slot; разные granularity |
...
### §5.1. Relative to PR #29 binary signatures ([`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md))

PR #29 binary signatures — текущая planned binary-surface (roadmap-only, not yet implemented; см. scope note в [`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md)). Это **chunk-level** fingerprints для bucket prefilter.

Три техники этого гайда — **orthogonal extensions** за пределами PR #29:

- **SAHI**: использует PR #29-style chunk codes как input; добавляет posting-layer поверх для retrieval. PR #29 — prefilter; SAHI — candidate generator + rerank.
- **ASMS**: работает на **document level**, не chunk level; PR #29 — chunk level. Разные granularity, не overlapping use cases.
...
Все три могут co-exist с PR #29 в одном стеке: PR #29 остаётся первым filter, три техники — alternatives/additions для second-stage retrieval.
...
| **M5-production** (hypothetical) | Possible stable contract, dual-read, integration with PR #29 binary signatures | Possible encoder versioning + dedup correction + agent memory benchmarks | Possible Pareto frontier + production model selection + integration with RAG pipeline |
...
- [`binary-embeddings-roadmap.md`](binary-embeddings-roadmap.md) — PR #29 binary signatures; current binary surface.
