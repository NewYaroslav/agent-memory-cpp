...
- **Bucket cardinality** ≈ `N / b` при равномерном распределении; растёт с ростом `N`.

#### 3.6.4. Worked example: b=32, r=2
...
| **IVF (FAISS IVFPQ)** | Clustering-based, batch-оптимизация, native metadata filtering, composes с PQ | High-load production с metadata фильтрами; billion-scale embeddings | Требует offline k-means training; медленнее для single-query низкой latency |
...
candidate set per query at b=32, r=2:
  expected candidates ≈ N × P(candidate | s)
  для s = 0.95 (relevant):  1M × ~1.00 ≈ 1M (all candidates — exhaustively re-ranked)
  для s = 0.18 (transition): 1M × 0.638 ≈ 638K (высокий recall, ~64% кандидатов)
  each candidate verified in O(d) = O(768) для exact cosine rerank
```

Trade-off vs HNSW: **faster index build, larger candidate set, more rerank work**. Trade-off vs IVF: **faster queries after warmup, simpler to update incrementally, no k-means training**. PRF integration: LSH — **candidate generator**, не ranker; кандидаты поступают в PRF pipeline (см. §6 этой секции).
