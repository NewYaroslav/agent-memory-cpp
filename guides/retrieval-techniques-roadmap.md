...
8 классов retrieval techniques по `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\11 RAG стратегий — спектр и комбинации.md` и `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Типы RAG - от Naive до Agentic.md`:
...
| **Hybrid** | sparse + dense + fusion (RRF или linear) | Практически всегда [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\11 RAG стратегий — спектр и комбинации.md`] | medium |
...
Эти классы не исключают друг друга. Production-система по `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\11 RAG стратегий — спектр и комбинации.md` рекомендует базовую комбинацию **Context-Aware Chunking + Hybrid Search + Reranking**, к которой добавляются другие классы по необходимости.
...
BM25 — Okapi BM25, канонический baseline retrieval. Вероятностная модель relevance с TF-сатурацией и length normalization. Формула скопирована из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\BM25 и BM25F — формулы и reference implementation.md`:
...
Контраст BM25 vs BM25F vs learned sparse приведён в источнике (`E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\BM25 и BM25F — формулы и reference implementation.md`):
...
Архитектурный паттерн SPLADE (из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Learned sparse retrieval — SPLADE семейство и гибридный sparse+dense.md`):
...
BGE-M3 — multi-functionality embedding (dense + sparse + ColBERT) в одной модели. Sparse branch наследует learned-sparse pattern. Соответствующий конвейер описан в `E:\_repoz\ai-agent-playbook\resources\rag-knowledge\ЮMoney Юджин корпоративный RAG на FRIDA и Milvus - разбор статьи.md` и `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Decoder-based retrievers — BGE и миграция от encoder.md`.

### 3.5. Nomic Embed v2 sparse

Nomic Embed Text v2 — sparse + dense + multi-vector. Источник `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Learned sparse retrieval — SPLADE семейство и гибридный sparse+dense.md` (см. таблицу семейства).
...
Decoder-based retrievers — переход от encoder-only (BERT) к decoder-only (LLM-style) архитектурам. Преимущества (из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Decoder-based retrievers — BGE и миграция от encoder.md`):
...
- **BGE-M3** — multi-functionality (dense + sparse + ColBERT) [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Decoder-based retrievers — BGE и миграция от encoder.md`].
...
Hard-negative mining — на этапе обучения bi-encoder учится отличать relevant documents от **похожих, но нерелевантных**. Источник: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Foundational RAG papers — BEIR, REALM, DPR, RAG.md` — DPR оригинал использует BM25 hard negatives.
...
EOS pooling победил на BEIR-бенчмарках для decoder-based архитектур [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Decoder-based retrievers — BGE и миграция от encoder.md`].
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Late-interaction retrieval — ColBERT архитектура и паттерн.md`:
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\BM25 и BM25F — формулы и reference implementation.md` trade-off таблицы: ColBERT даёт quality 0.52-0.56 BEIR nDCG@10 при storage cost high.
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\PRF interpolation timing - Pre-PRF vs Post-PRF vs Both-PRF.md`, источник: Li et al. SIGIR 2022 ("To Interpolate or not to Interpolate", arXiv 2205.00235):
...
Anthropic-техника обогащения chunks контекстом документа перед индексацией. Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Contextual Retrieval — LLM-обогащение чанков контекстом документа.md`:
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Contextual Retrieval — LLM-обогащение чанков контекстом документа.md`:
...
EMNLP 2025 paper: «LLM-Independent Adaptive RAG» (Maria Marina et al., 2025). Источник: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Adaptive RAG — lightweight routing без LLM.md`.
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\11 RAG стратегий — спектр и комбинации.md` (производственный конвейер):
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Multimodal RAG - VLM для анализа картинок и графиков.md`:

- Hybrid text+image retrieval.
- VLM понимает графики, не OCR.
- Арифметика — без отдельного калькулятора.

В нашем стеке multimodal RAG — **M2+** extension. Docling-адаптер как `IResourceAdapter` может извлекать и текст, и embedded images; VLM — external model.
...
Из `E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md` (секция 4):
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Event-based RAG и динамический граф событий.md`:
...
Из `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md`:
...
Из `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Deterministic RAG vs autonomous RAG-агенты.md`:
...
RLM — root LM в Python REPL, которая пишет код для исследования контекста и вызывает sub-RLM для фрагментов. Из `E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md` (секция 11):
...
Из `E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md`:
...
- **Docling multimodal** — `IResourceStore` adapter для PDF/DOCX/PPTX/XLSX/images/audio/HTML.
...
2. **α для linear interpolation vs RRF.** RRF не требует cross-backend score normalization, но linear interpolation может дать лучший top-K [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\PRF interpolation timing - Pre-PRF vs Post-PRF vs Both-PRF.md`]. Нужны head-to-head benchmarks.
3. **Both-PRF в нашем HybridRetriever.** PRF не реализовано в текущем контракте; Both-PRF даёт 1-3% nDCG@10 vs Post-PRF, но стоимость двойной работы [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\PRF interpolation timing - Pre-PRF vs Post-PRF vs Both-PRF.md`]. Стоит ли реализовывать через `IQueryTransformer`?
4. **Anthropic 49% reduction на русском корпусе.** Не воспроизведено [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Contextual Retrieval — LLM-обогащение чанков контекстом документа.md`]. Наша документация преимущественно русскоязычная — нужен ли пилот?
5. **Anti-loop formula для не-live workloads.** Cooldown+decay по СВИНОПАС разработан для voice-streamed chat [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md`]. Какой decay curve оптимален для batch retrieval?
6. **Spreading activation depth & decay-per-hop.** lifemodel не публикует конкретные параметры [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md`]. Нужны работы на наших корпусах.
...
8. **Bi-temporal validation.** Zep использует bi-temporal; наш `TemporalComponent.valid_from_ms`/`valid_until_ms` одномерный [Source: `E:\_repoz\ai-agent-playbook\tools\ai-agents\Zep Graphiti — temporal knowledge graph для AI-агентов (документация).md`]. Нужно ли расширять для legal/compliance workloads?
...
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\BM25 и BM25F — формулы и reference implementation.md` — канонические формулы, Robertson & Zaragoza 2009, Pérez-Iglesias Lucene reference.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Learned sparse retrieval — SPLADE семейство и гибридный sparse+dense.md` — SPLADE v1/v2/v2, BGE-M3 sparse branch, Nomic Embed v2 sparse.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\PRF interpolation timing - Pre-PRF vs Post-PRF vs Both-PRF.md` — SIGIR 2022 Li et al., interpolation + PRF decision matrix.

**Dense retrieval:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Foundational RAG papers — BEIR, REALM, DPR, RAG.md` — DPR, REALM, BEIR, hard-negative mining.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Decoder-based retrievers — BGE и миграция от encoder.md` — BGE-M3, Qwen-Embed, EOS pooling, multi-functionality.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Vector DB для RAG - выбор и алгоритмы ANN.md` — ANN algorithms (HNSW/FAISS/DiskANN/ScaNN).
- `E:\_repoz\ai-agent-playbook\concepts\llm-research\ANN-алгоритмы для векторного поиска — HNSW, FAISS, DiskANN, ScaNN, фильтрация.md` — детали ANN-выбора.

**Late interaction:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Late-interaction retrieval — ColBERT архитектура и паттерн.md` — ColBERT v1/v2/v3, PLAID, RAGatouille, Vespa multi-vector.

**Contextual Retrieval:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Contextual Retrieval — LLM-обогащение чанков контекстом документа.md` — Anthropic техника, 49-67% reduction, $0.50/1000 chunks.
- `E:\_repoz\ai-agent-playbook\playbooks\rag\Contextual Retrieval — индексация production-RAG с Haiku + prompt caching.md` — playbook с Haiku + prompt caching.
- `E:\_repoz\ai-agent-playbook\resources\n8n-automation\Метод улучшения RAG от Anthropic - Контекстуализация в n8n - расшифровка.md` — реализация через n8n + Qdrant.

**Multimodal RAG:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Multimodal RAG - VLM для анализа картинок и графиков.md` — VLM для графиков, диаграмм, таблиц.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\11 RAG стратегий — спектр и комбинации.md` — production-stack по devclubspb с VLM упоминанием.

**Graph retrieval:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Event-based RAG и динамический граф событий.md` — DyG-RAG style, temporal anchors.
- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md` — spreading activation pattern.
- `E:\_repoz\ai-agent-playbook\resources\llm-research\A-MEM - Agentic Memory for LLM Agents (arXiv 2502.12110) - разбор статьи.md` — A-MEM link generation, memory evolution.

**Adaptive routing:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Adaptive RAG — lightweight routing без LLM.md` — EMNLP 2025 LLM-Independent Adaptive RAG, 27 features.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Deterministic RAG vs autonomous RAG-агенты.md` — APTUS critique, deterministic LangGraph.

**RLM:**

- `E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md` — раздел RLM, RLM vs RAG failure modes.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Агентный RAG vs традиционный RAG - эволюция подхода к поиску.md` — Agentic RAG spectrum.

**Memory frameworks (cross-ref):**

- `E:\_repoz\ai-agent-playbook\tools\ai-agents\Mem0 — open-source memory layer для LLM-приложений (документация).md` — multi-signal scoring (semantic + BM25 + entity).
- `E:\_repoz\ai-agent-playbook\tools\ai-agents\Zep Graphiti — temporal knowledge graph для AI-агентов (документация).md` — bi-temporal architecture.
- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md` — anti-loop formulas, floating subgraph.

**Misc:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\RAG без эмбеддингов — LLM-классификация страниц вместо similarity search.md` — alternative page classification.
