...
где `s = sim(q, x)`, `b` = bands, `r` = rows per band, общее число хешей `b × r`.
...
S-curve `P(candidate | s) = 1 - (1 - s^r)^b` — primary trade-off LSH: **число кандидатов** vs **precision** vs **threshold**. **Transition point** (где S-curve переключается с low к high):

```text
s* = (1/b)^(1/r)
```

В точке `s = s*` вероятность кандидатства `P(candidate | s*) = 1 - (1 - 1/b)^b ≈ 1 - 1/e ≈ 0.632` (для больших `b`; для малых `b` отклонение невелико). **Это ~63%, не 100%** — soft probabilistic contract, не hard threshold. Per PR #31 round 2 review lesson: reviewer explicitly flagged, что LSH is **not** a hard threshold.
...
#### 3.6.4b. Worked example (random-hyperplane / cosine): b=64, r=2 (128-bit)

Для cosine-подобных векторов LSH hash family — sign-of-random-projection. Здесь `s` в общей S-curve формуле `P(candidate | s) = 1 - (1 - s^r)^b` интерпретируется как **per-bit collision probability**, не прямое cosine similarity. Per-bit collision probability для случайной гиперплоскости:

```text
Random-hyperplane LSH: b=64, r=2 (for 128-bit code, since 64 × 2 = 128)

Per-bit collision probability: p_bit(c) = 1 - arccos(c) / π
At cosine similarity c = 0.95:
  p_bit(0.95) = 1 - arccos(0.95) / π ≈ 1 - 0.3176/π ≈ 0.899

P(candidate | c=0.95) = 1 - (1 - p_bit^r)^b = 1 - (1 - 0.899²)^64 ≈ 1 (≈1.0)

At cosine c = 0.5:
  p_bit(0.5) = 1 - arccos(0.5)/π = 1 - (π/3)/π = 1 - 1/3 ≈ 0.667
  P(candidate | c=0.5) = 1 - (1 - 0.667²)^64 ≈ 1 - 0.0017 ≈ 0.998

At cosine c = 0.18:
  p_bit(0.18) = 1 - arccos(0.18)/π ≈ 1 - 1.39/π ≈ 0.558
  P(candidate | c=0.18) = 1 - (1 - 0.558²)^64 ≈ 1 - 0.00002 ≈ 0.99998

Comparison: MinHash at J=0.177 ≈63.8% (high false positive rate at transition). Random-hyperplane at c=0.18 ≈99.99% (transition much earlier because p_bit(c) is higher than c).
```
...
candidate set per query at b=64, r=2 (128-bit raw code):
  E[candidates] = Σ_i P(candidate | s_i)

  This only simplifies to N × P(candidate | s) in the artificial case
  where ALL N objects have the same similarity s to the query. Real
  corpora have a distribution of similarities, so for high s (e.g.,
  0.95) very few objects have such high similarity — expected candidates
  is typically a small fraction of N, not N × 0.999 ≈ N.

  For sanity-check: with 1M 768-dim embeddings, even if 1000 objects
  have cosine 0.95 to the query, expected candidates ≈ 1000 × ~1.0 ≈ 1000
  (not 1M × ~1.0 = 1M).
