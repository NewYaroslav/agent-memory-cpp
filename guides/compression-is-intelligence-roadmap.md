...
В этом смысле **LLM pretraining = next-token prediction = compression of training corpus**. Модель учится не просто «угадывать слова», она учится строить компактную вероятностную модель языка, которая захватывает статистические регулярности корпуса.
...
То есть high-entropy chunk (редкое содержание) — это **деталь**, которая меняет ответ; low-entropy chunk (общий смысл) — это **шум**, который повторяет очевидное.
