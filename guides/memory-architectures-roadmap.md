...
│       [Source: E:\_repoz\ai-agent-playbook\concepts\ai-agents\Блок фактов vs RAG-память.md]
│
├─ Massive long-context (10K-200K), known structure
│    → Long-context frontier models без memory layer
│       [Source: E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md]
│
├─ Live-streaming с ASR-шумом + anti-loop requirement
│    → СВИНОПАС (anti-loop + spreading activation)
│       [Source: E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md]
│
├─ Personal Obsidian с manual hierarchy + selective embedding
│    → NOUZ (LUCA/PRIZMA/SLOI) или Vault Audit AI (LLM-driven)
│       [Source: E:\_repoz\ai-agent-playbook\concepts\NOUZ — структурированная память для ИИ-агентов в Obsidian.md]
│
├─ Personal assistant, evolving preferences, no schema
│    → A-MEM (Zettelkasten-based)
│       [Source: E:\_repoz\ai-agent-playbook\concepts\ai-agents\Zettelkasten-based agentic memory — A-MEM pattern.md]
│
├─ Multi-tenant SaaS, persistent memory, structured facts
│    → Mem0 (semantic + BM25 + entity, 4 scopes)
│       [Source: E:\_repoz\ai-agent-playbook\tools\ai-agents\Mem0 — open-source memory layer для LLM-приложений (документация).md]
│
├─ Customer history с temporal ordering
│    → Zep / Graphiti (temporal knowledge graph)
│       [Source: E:\_repoz\ai-agent-playbook\tools\ai-agents\Zep Graphiti — temporal knowledge graph для AI-агентов (документация).md]
│
├─ Persistent chat-agent с очень длинной историей (LM-managed paging)
│    → Letta (new letta-code stack) или MemGPT
│       [Source: E:\_repoz\ai-agent-playbook\tools\ai-agents\Letta (бывш. MemGPT) — stateful agents с advanced memory.md]
│
├─ Industry-grade personalization с weighted-KG user modeling
│    → Memoria (session summarization + weighted KG)
│       [Source: E:\_repoz\ai-agent-playbook\resources\llm-research\Memoria - Scalable Agentic Memory Framework - конспект.md]
│
├─ User-side modelling с evidence chain (DLP-bridge pattern)
│    → memory_claim / mental_hypothesis / ToM sidecar
│       [Source: E:\_repoz\ai-agent-playbook\concepts\ai-agents\DLP-подобная архитектура профилирования для AI-агента.md]
│
├─ Production кодовая база с live pipeline + multi-dimensional backlog
│    → Исполняемый граф контекста (AST + Git + sessions + obligations)
│       [Source: E:\_repoz\ai-agent-playbook\concepts\ai-agents\Исполняемый граф контекста - AST Git sessions logs obligations.md]
│
├─ Multi-agent cross-session wiki + 6-hour maintaic cron
│    → Карпати 3-layer + Wiki-maintainer
│       [Source: E:\_repoz\ai-agent-playbook\concepts\ai-agents\Трёхслойная память для AI-агентов - метод Карпати.md]
│
├─ Privacy-first, no API, >100K entities, structural memory
│    → lifemodel dual-layer (LanceDB + custom graph + Transformers.js)
│       [Source: E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md]
│
...
└─ Минимальный уровень: AGENTS.md / skills / RAG
     [Source: E:\_repoz\ai-agent-playbook\concepts\ai-agents\Уровни памяти AI-агента — от контекста до fine-tuning.md]
```
...
**Где:** СВИНОПАС (ограничение области поиска текущим контекстом) [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md`], lifemodel (spreading activation от seed-узла) [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md`], Исполняемый граф (live-pipeline narrowing под задачу) [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Исполняемый граф контекста - AST Git sessions logs obligations.md`].
...
**Где:** lifemodel (от seed-узла активация распространяется по соседним рёбрам с убывающим весом) [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md`], A-MEM (links с LLM-judged relationship types → effective spreading через multi-hop links).
...
**Где:** lifemodel (salience decay параметры не раскрыты в статье) [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md`], СВИНОПАС (cooldown + decay по use-count + формулы релевантности) [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md`], Карпати wiki archive (`injection_count=0` и `last_updated > 60d` → archive) [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Трёхслойная память для AI-агентов - метод Карпати.md`].
...
**Где:** Mem0 (semantic + BM25 + entity, fused by score) [Source: `E:\_repoz\ai-agent-playbook\tools\ai-agents\Mem0 — open-source memory layer для LLM-приложений (документация).md`], Карпати (raw conversations + Mem0-facts + wiki-articles) [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Трёхслойная память для AI-агентов - метод Карпати.md`], Memoria (session summary + weighted KG) [Source: `E:\_repoz\ai-agent-playbook\resources\llm-research\Memoria - Scalable Agentic Memory Framework - конспект.md`].
...
**Где:** lifemodel [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md`], Zep Graphiti (semantic + keyword + graph-based retrieval) [Source: `E:\_repoz\ai-agent-playbook\tools\ai-agents\Zep Graphiti — temporal knowledge graph для AI-агентов (документация).md`], Mem0 (semantic + BM25 + entity) [Source: `E:\_repoz\ai-agent-playbook\tools\ai-agents\Mem0 — open-source memory layer для LLM-приложений (документация).md`].
...
**Где:** DLP-bridge pattern [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\DLP-подобная архитектура профилирования для AI-агента.md`] — отдельный sidecar LLM-call возвращает structured JSON (intent, constraints, style).
...
**Где:** A-MEM [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Zettelkasten-based agentic memory — A-MEM pattern.md`], NOUZ (manual linking) [Source: `E:\_repoz\ai-agent-playbook\concepts\NOUZ — структурированная память для ИИ-агентов в Obsidian.md`], Карпати Wiki (frontmatter-driven hierarchy).
...
|---|---|
| **J-Space** | In-model emergent state. Не хранилище, а active workspace во время forward pass — не реализуемо на нашем substrate. [Source: `E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md`] |
| **RLM** | Контекст в Python REPL с root LM, которая пишет код для исследования. Sub-LM вызовы + REPL execution — другой execution model. [Source: `E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md`] |
| **Исполняемый граф контекста** | Runtime multi-node graph (AST + Git + AI-сессии + obligations + …) с N-мерной backlog surface — модель данных, не retrieval layer. [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Исполняемый граф контекста - AST Git sessions logs obligations obligations.md`] |
| **Letta / MemGPT** | LM-managed page-in/page-out — отдельная модель memory, не совместимая с нашим scope-aware key model (LM должен «знать» про secondary indexes, MDBX layout). [Source: `E:\_repoz\ai-agent-playbook\tools\ai-agents\Letta (бывш. MemGPT) — stateful agents с advanced memory.md`] |
| **СВИНОПАС (full)** | Live-streaming ASR + Гном Гномыч fine-tuned extractor + кастомный датабазч — full proprietary stack. Часть (anti-loop, spreading activation) переиспользуема, часть — нет. [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md`] |
...
3. **Spreading activation depth.** Какой depth оптимален для разных workloads? lifemodel не публикует конкретные параметры [Source: `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md`].
4. **Cyrillic anti-loop.** СВИНОПАС специально использует лемматизацию/стемминг для русского + alias resolution. Нужно ли это для нашего стека на уровне archetype или на уровне per-stack (например, для Russian-language `BasicRag`)? [Source: `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md`]
...
- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Трёхслойная память для AI-агентов - метод Карпати.md` — 3-layer memory + 4-й слой Obsidian, wiki-maintainer, anti-мусор механика.
- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Уровни памяти AI-агента — от контекста до fine-tuning.md` — 7-tier hierarchy + 6.5 RLM + chasing_nlp classification.

**Schema-less & evolution:**

- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Zettelkasten-based agentic memory — A-MEM pattern.md` — A-MEM pattern: atomic notes + LLM-driven linking + memory evolution.
- `E:\_repoz\ai-agent-playbook\resources\llm-research\A-MEM - Agentic Memory for LLM Agents (arXiv 2502.12110) - разбор статьи.md` — A-MEM original paper (NeurIPS 2025).

**Structured / frontmatter / Obsidian-native:**

- `E:\_repoz\ai-agent-playbook\concepts\NOUZ — структурированная память для ИИ-агентов в Obsidian.md` — LUCA/PRIZMA/SLOI tiers, embedding anisotropy problem.
- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Self-Evolving Memory для Claude Code на основе LLM Knowledge Base.md` — Cole Medin + Karpathy compiler analogy + Compounding Knowledge Loop.

**Lightweight / flat:**

- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Блок фактов vs RAG-память.md` — Блок фактов (system prompt, no retrieval, ≤100 facts).

**Vector + graph overlay:**

- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Dual-layer memory retrieval LanceDB и spreading activation graph.md` — lifemodel dual-layer, salience decay, spreading activation, plugin-изоляция.

**Live-agents & anti-loop:**

- `E:\_repoz\ai-agent-playbook\concepts\rag-knowledge\Внешняя память LLM-агентов — система СВИНОПАС.md` — Гном Гномыч, граф сущностей, anti-loop формулы.

**User modelling / ToM:**

- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\DLP-подобная архитектура профилирования для AI-агента.md` — ToM sidecar, evidence chain, ethical boundaries.

**Multi-node context graph:**

- `E:\_repoz\ai-agent-playbook\concepts\ai-agents\Исполняемый граф контекста - AST Git sessions logs obligations.md` — multi-dimensional backlog, N-D surface.

**Memory frameworks (comparison & docs):**

- `E:\_repoz\ai-agent-playbook\concepts\llm-research\Сравнение подходов организации памяти и контекста LLM.md` — Long context → Naive RAG → MemGPT → Mem0 → Zep → RLM → A-MEM спектр; J-Space как in-model layer.
- `E:\_repoz\ai-agent-playbook\tools\ai-agents\Mem0 — open-source memory layer для LLM-приложений (документация).md` — Mem0 OSS components, two deployment modes (library / self-host).
- `E:\_repoz\ai-agent-playbook\tools\ai-agents\Letta (бывш. MemGPT) — stateful agents с advanced memory.md` — Letta repository split (V1 legacy vs letta-code active), SDKs.
- `E:\_repoz\ai-agent-playbook\tools\ai-agents\Zep Graphiti — temporal knowledge graph для AI-агентов (документация).md` — Graphiti bi-temporal, 4 backends (Neo4j/FalkorDB/Neptune/Kuzu).

**Industry:**

- `E:\_repoz\ai-agent-playbook\resources\llm-research\Memoria - Scalable Agentic Memory Framework - конспект.md` — Memoria weighted-KG + session summarization.
