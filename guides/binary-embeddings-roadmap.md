...
| PR #80 | Evaluate cluster-local/document-local projections only as hierarchical second-stage filters. | Show value after global routing, with query encoding cost counted per selected cluster/document. | No global comparison of incompatible local codes. |
| PR #81 | Scale and lifecycle benchmark gate before production backend selection. | Add a small CI-validated lifecycle report for exact vector, flat binary, and multi-probe binary indexes: build/query timing, payload bytes, candidate coverage, update/delete, rebuild, and bucket diagnostics. | No production backend decision, no persisted reload claim, and no stable large-scale benchmark result from a tiny smoke fixture. |
| PR #82+ | Choose index/backend direction: production binary bucket, MIH/HNSW-Hamming, IVF/PQ hybrid, HNSW dense baseline, or asymmetric quantization path. | Sub-linear candidate generation or clearly lower memory-bandwidth cost at target recall and lifecycle cost. | More flat-scan polishing unless a benchmark justifies it. |
