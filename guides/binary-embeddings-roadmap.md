...
| PR #73 | Actual third-party embedding-model fixture when generator environment is stable. | Compare Hadamard/PCA/ITQ/float on frozen vectors from a pinned model/tokenizer/provider with recorded generator command/config. | Network/model generation in CI unless the dependency is cheap, pinned, and reproducible. |
